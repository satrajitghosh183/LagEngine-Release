...
extern "C" void updateParticleSystem(cudaGraphicsResource_t particleResource,
                                     ...
                                     cudaStream_t stream) {
    // Map resources on the specified stream
    cudaGraphicsMapResources(1, &particleResource, stream);
    
    Particle* d_particles = nullptr;
    size_t numBytes = 0;
    cudaGraphicsResourceGetMappedPointer((void**)&d_particles, &numBytes, particleResource);
    ...
    if (!g_particlesInitialized) {
        ...
        cudaStreamSynchronize(stream);  // Sync only this stream
    ...
    // Unmap resources on the specified stream
    cudaGraphicsUnmapResources(1, &particleResource, stream);
}

...
n_mid = 60
pose_mid = poses_tar[n_mid].cpu().numpy()
poses_rest = util.get_standard_poses_from_tar_pose(pose_mid, N_views=NM-n_mid)
poses_tar[n_mid:NM] = torch.Tensor(poses_rest).to(device=device)
...
vid_depth_path = os.path.join(save_path, vid_name + "_depth.mp4")

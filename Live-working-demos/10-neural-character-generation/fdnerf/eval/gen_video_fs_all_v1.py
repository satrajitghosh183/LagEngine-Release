...
args, conf = util.args.parse_args(
    ...
    default_conf="conf/exp/fs_multiexp_2D_Cat.conf",
...
NM = len(dset)
print("All test cases are", NM)
for mid in range(0, NM, 100):
    data = dset[mid]
    ...
    len_sem = 73
    input_semantic = data["semantic_cdn"][:args.nviews, :len_sem].to(
        device=device)
    # input_semantic = data["semantic_cdn"].to(device=device)
    ...
    if args.scale != 1.0:
        Ht = int(H * args.scale)
        Wt = int(W * args.scale)
        if abs(Ht / args.scale - H) > 1e-10 or abs(Wt / args.scale -
                                                   W) > 1e-10:
            warnings.warn(
    ...
    if get_spiral_poses:
        ...
        render_poses = torch.stack(
            [
                ...
                for angle in np.linspace(-180, 180, args.num_video_frames +
                                         1)[:-1]
            ],
            0,
        )  # (NV, 4, 4)
    ...
    with torch.no_grad():
        ...
        for rays in tqdm.tqdm(
                torch.split(render_rays.view(-1, 8),
                            args.ray_batch_size,
                            dim=0)):
    ...
    vid_name = args.split + "{:04}".format(mid)
    ...
    print("Writing Image")
    viewimg_in_path = os.path.join(save_path,
                                   "video" + vid_name + "_view_in.jpg")
    ...
    imageio.imwrite(viewimg_in_path,
                    (img_in_out * 255).numpy().astype(np.uint8))

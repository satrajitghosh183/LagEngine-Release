"""
Author: Eckert ZHANG
Date: 2022-03-17 18:08:17
LastEditTime: 2022-03-17 18:18:26
...
def render_path_spiral(c2w, up, rads, focal, zdelta, zrate, rots, N):
    ...
    for theta in np.linspace(0., 2. * np.pi * rots, N + 1)[:-1]:
        c = np.dot(
            ...
            np.array(
                [-np.sin(theta),
                 np.cos(theta), -np.sin(theta * zrate), 1.]) * rads)
...
def generate_render_path(poses, bds, comps=None, N=30):
    ...
    if comps[0]:
        render_poses += render_path_axis(c2w, up, 1, shrink_factor * rads[1],
                                         focal, N)
    if comps[1]:
        render_poses += render_path_axis(c2w, up, 0, shrink_factor * rads[0],
                                         focal, N)
    if comps[2]:
        render_poses += render_path_axis(c2w, up, 2, shrink_factor * zdelta,
                                         focal, N)

    rads[2] = zdelta
    if comps[3]:
        render_poses += render_path_spiral(c2w, up, rads, focal, zdelta, 0., 1,
                                           N * 2)
    if comps[4]:
        render_poses += render_path_spiral(c2w, up, rads, focal, zdelta, .5, 2,
                                           N * 4)

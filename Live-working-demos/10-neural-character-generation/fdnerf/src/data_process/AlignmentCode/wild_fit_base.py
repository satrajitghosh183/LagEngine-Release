...
glob_neutral_tmp_LM = np.array([[143, 214], [146, 244], [151, 273], [158, 302],
                                [168, 328], [184, 352], [205, 371], [229, 386],
                                [259, 390], [287, 385], [311, 371], [331, 352],
                                [347, 329], [356, 303], [364, 274], [368, 245],
                                [370, 214], [163, 186], [177, 172], [197, 168],
                                [218, 173], [236, 182], [276, 180], [296, 168],
                                [317, 163], [339, 167], [352, 184], [257, 206],
                                [257, 226], [257, 246], [257, 267], [237, 286],
                                [248, 288], [258, 289], [268, 288], [278, 285],
                                [183, 210], [195, 203], [210, 204], [223, 215],
                                [209, 217], [194, 217], [290, 213], [303, 203],
                                [318, 201], [330, 207], [320, 214], [305, 215],
                                [219, 328], [234, 320], [249, 314], [258, 317],
                                [268, 314], [281, 320], [295, 328], [282, 338],
                                [269, 342], [258, 343], [248, 343], [234, 339],
                                [226, 328], [248, 326], [258, 327], [268, 326],
                                [289, 328], [268, 327], [258, 328], [248,
                                                                     327]])
...
class fitting:
    ...
    def detect_kp2d(self,
        ...
        # ========== extract landmarks ==========
        # detect faces
        faces = self.detector(src_img, 1)
        # get landmarks for the first face
        pts = self.face_pred(src_img, faces[0])
        # shape=[68,2]
        ...
        if tar_img is not None:
            # detect faces
            faces = self.detector(tar_img, 1)
            # get landmarks for the first face
            pts = self.face_pred(tar_img, faces[0])
        ...
        M, scale = self.transformation_from_points(src_points=kp2d_raw,
                                                   tmpt_points=kp2d_tmpl)
        if is_rigid:
            # np.sqrt(np.sum(np.array(M[0, :2])**2))
            # np.sqrt(np.sum(np.array(M[1, :2])**2))
            scale_x = (M[0, 0] + M[1, 1]) / 2.
            scale_y = scale_x
    ...
    def transformation_from_points(self, src_points, tmpt_points):
        ...
        return np.vstack([
            np.hstack([
                (s2 / s1) * R, (c2.T - (s2 / s1) * R.dot(c1.T)).reshape(-1, 1)
            ]),  # 2,4   ??
            np.matrix([0., 0., 1.])
        ]), s2 / s1
...
class pose_estimate():
    def __init__(self):
        landmark = np.load(
            "./src/data_process/AlignmentCode/1_975_landmarks.npy")
    ...
    def fit_kp2d(self, kp2d):
        ...
        tmpLM_2D = self.project(self.tmpLM, rot_vector, scale,
                                trans)  # landmark 3d -> 2d

        # ========== iterative optimize ==========
        for optimize_loop in range(50):
            ...
            lm = lm_pos - lm_mean
            scale = np.sum(np.linalg.norm(lm, axis=1)) / np.sum(
                np.linalg.norm(vertices_2d, axis=1))
            trans = lm_mean - vertices_mean * scale
            lm_pos_3D = self.tmpLM
            scale, trans, rot_vector = self._optimize_rigid_pos_2d(
                scale, trans, rot_vector, lm_pos_3D, lm_pos)
    ...
    def from_kp2d_get_pos(self, kp2d):
        ...
        tmpLM_2D = self.project(self.tmpLM, rot_vector, scale,
                                trans)  # landmark 3d -> 2d
        # ========== iterative optimize ==========
        for optimize_loop in range(50):  # 4
            tmpLM_2D = tmpLM_2D / scale
            vertices_mean = np.mean(tmpLM_2D, axis=0)
            vertices_2d = tmpLM_2D - vertices_mean
        ...
        camRT = R.T.dot(np.array(
            [[0, -1, 0], [1, 0, 0],
             [0, 0, 1]]))  # first to trans the axis,  second to R->RT
    ...
    def convert_rot_vector(self, rot_vec):
        ...
        t = np.array([[0, -v[2], v[1]], [v[2], 0, -v[0]], [-v[1], v[0], 0]],
                     dtype=np.float32)
        R = np.cos(theta) * (np.eye(3)) + (1 - np.cos(theta)) * (v).dot(
            v.T) + np.sin(theta) * t
        return R

    # ================================= inner functions ==================================
    def _optimize_rigid_pos_2d(self, scale, trans, rot_vector, lm_pos_3D,
                               lm_pos):
        lm_pos_3D = lm_pos_3D.ravel()
        lm_pos = lm_pos.ravel()
        params = np.hstack((scale, trans, rot_vector))
        result = least_squares(self._compute_res_rigid,
                               params,
                               verbose=0,
                               x_scale='jac',
                               ftol=1e-5,
    ...
    def project(self,
                ...
                scale,
                trans,
                keepz=False):  # 这就是一个正交投影的过程，成像面在xy上。
...
def get_image_hull_mask(image_shape, image_landmarks, ie_polys=None):
    ...
    cv2.fillConvexPoly(
        hull_mask,
        cv2.convexHull(np.concatenate((int_lmrks[0:9], int_lmrks[17:18]))),
        (1, ))

    cv2.fillConvexPoly(
        hull_mask,
        cv2.convexHull(np.concatenate((int_lmrks[8:17], int_lmrks[26:27]))),
        (1, ))

    cv2.fillConvexPoly(
        hull_mask,
        cv2.convexHull(np.concatenate((int_lmrks[17:20], int_lmrks[8:9]))),
        (1, ))

    cv2.fillConvexPoly(
        hull_mask,
        cv2.convexHull(np.concatenate((int_lmrks[24:27], int_lmrks[8:9]))),
        (1, ))
    ...
    cv2.fillConvexPoly(
        hull_mask,
        cv2.convexHull(
            np.concatenate((int_lmrks[17:22], int_lmrks[27:28],
                            int_lmrks[31:36], int_lmrks[8:9]))), (1, ))

    cv2.fillConvexPoly(
        hull_mask,
        cv2.convexHull(
            np.concatenate((int_lmrks[22:27], int_lmrks[27:28],
                            int_lmrks[31:36], int_lmrks[8:9]))), (1, ))

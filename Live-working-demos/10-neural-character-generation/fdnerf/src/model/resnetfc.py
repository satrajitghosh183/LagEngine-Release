...
LastEditTime: 2021-12-16 22:12:45
...
class ResnetFC_Indep(nn.Module):
    """
    Different from the original ResnetFC, here, the position and direction are respectively fed into the net.
    """
    def __init__(
        self,
        d_pos_in,
        ...
        self.lin_pos_in = nn.Linear(d_pos_in, d_hidden)
        nn.init.constant_(self.lin_pos_in.bias, 0.0)
        nn.init.kaiming_normal_(self.lin_pos_in.weight, a=0, mode="fan_in")
        ...
        if d_dir_in == 0:
            ...
            self.lin_alpha_out = nn.Linear(d_hidden, 1)
            self.lin_feature = nn.Linear(d_hidden, d_hidden)
            self.lin_rgb_out = nn.ModuleList([nn.Linear(d_hidden+d_dir_in, d_hidden // 2), nn.Linear(d_hidden // 2, 3)])
            nn.init.constant_(self.lin_alpha_out.bias, 0.0)
            nn.init.kaiming_normal_(self.lin_alpha_out.weight, a=0, mode="fan_in")
            nn.init.constant_(self.lin_feature.bias, 0.0)
            nn.init.kaiming_normal_(self.lin_feature.weight, a=0, mode="fan_in")
            for i in range(2):
                nn.init.constant_(self.lin_rgb_out[i].bias, 0.0)
                nn.init.kaiming_normal_(self.lin_rgb_out[i].weight, a=0, mode="fan_in")

        self.d_pos_in = d_pos_in
        self.d_dir_in = d_dir_in
        self.d_in = d_pos_in + d_dir_in
        self.n_blocks = n_blocks
        self.d_latent = d_latent
        self.d_out = d_out
        self.d_hidden = d_hidden
    ...
    def forward(self,
        ...
        with profiler.record_function("resnetfc_infer"):
            ...
            if self.d_latent > 0:
                z = zx[..., :self.d_latent]
                p_x = zx[..., self.d_latent:self.d_latent+self.d_pos_in]
                d_x = zx[..., -self.d_dir_in:]
            else:
                p_x = zx[..., :self.d_pos_in]
                d_x = zx[..., -self.d_dir_in:]

            x = self.lin_pos_in(p_x)
            for blkid in range(self.n_blocks):
                if blkid == self.combine_layer:
                    x = util.combine_interleaved(x, combine_inner_dims,
                                                 self.combine_type)
                    d_x = util.combine_interleaved(d_x, combine_inner_dims,
                                                 self.combine_type)
            ...
            if self.d_dir_in == 0:
                ...
                x = x.reshape(-1, self.d_latent)
                alpha = self.lin_alpha_out(self.activation(x))
                fea = self.lin_feature(self.activation(x))
                b, ns, _ = d_x.shape
                x = torch.cat([fea, d_x.reshape(b*ns,-1)], -1)
                for i in range(2):
                    x = self.lin_rgb_out[i](x)
                    if i == 0:
                        x = self.activation(x)
                out = torch.cat([x, alpha], -1)
            return out

    @classmethod
    def from_conf(cls, conf, d_pos_in, d_dir_in, **kwargs):
        # PyHocon construction
        return cls(
            d_pos_in,
            d_dir_in,

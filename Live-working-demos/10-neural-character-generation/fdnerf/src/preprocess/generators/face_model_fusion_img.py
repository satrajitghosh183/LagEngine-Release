"""
Author: Eckert ZHANG
Date: 2021-10-27 16:30:06
LastEditTime: 2021-11-11 11:10:16
LastEditors: Eckert ZHANG
FilePath: /PIRender/generators/face_model_fusion_img.py
Description: 
"""
import functools
import numpy as np
...
from preprocess.util import flow_util
from preprocess.generators.base_function import LayerNorm2d, ADAINHourglass, FineEncoder, FineDecoder
...
class EditingNet(nn.Module):
    def __init__(self, image_nc, descriptor_nc, layer, base_nc, max_nc,
        ...
        # encoder part
        self.img_num = 3
        self.encoder = FineEncoder(image_nc * 2 * self.img_num, base_nc,
                                   max_nc, layer, **kwargs)
        self.decoder = FineDecoder(image_nc * self.img_num, self.descriptor_nc,
                                   base_nc, max_nc, layer, num_res_blocks,
                                   **kwargs)
    ...
    def forward(self, input_image, warp_image, descriptor):
        ...
        x = torch.stack([
            x[:self.img_num].view(-1, H, W), x[-self.img_num:].view(-1, H, W)
        ])
        # d1,d2=torch.chunk(descriptor,2)
        descriptor = torch.stack(
            [descriptor[:1].view(-1, 1), descriptor[-1:].view(-1, 1)])

        x = self.encoder(x)
        gen_image = self.decoder(x, descriptor)

        gen_image = torch.cat(
            [gen_image[0].view(-1, 3, H, W), gen_image[1].view(-1, 3, H, W)],
            0)
        return gen_image

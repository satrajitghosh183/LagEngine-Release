...
LastEditTime: 2022-03-31 17:50:31
...
from preprocess_v2.generators.swin_transformer import SwinTransformer3D_Encoder
...
class EditingNet(nn.Module):
    ...
    def forward(self, input_image, warp_image, descriptor):
        ...
        # # --> B, C, D, H, W
        # in_imgs = input_image.reshape(-1, 3, C, W, H).permute(0, 2, 1, 3, 4)
        # warp_imgs = warp_image.reshape(-1, 3, C, W, H).permute(0, 2, 1, 3, 4)
        # in_warps = torch.cat([in_imgs, warp_imgs], dim=2)
        # out = self.swin3D(in_warps)
        x = torch.cat([input_image, warp_image], 1)
        # x = self.encoder(x)
        x = self.swin3D_encoder(
            x.reshape(-1, 3, 2 * C, W, H).permute(0, 2, 1, 3, 4))

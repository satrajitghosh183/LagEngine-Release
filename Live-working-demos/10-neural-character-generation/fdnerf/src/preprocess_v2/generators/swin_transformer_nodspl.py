...
LastEditTime: 2022-03-31 17:47:18
...
# from preprocess_v2.generators.swin_trans_builder import BACKBONES
...
class PatchMerging(nn.Module):
    ...
    Args:
        ...
        norm_layer (nn.Module, optional): Normalization layer.  Default: nn.LayerNorm
    """
    def __init__(self, dim, dim_max=256, norm_layer=nn.LayerNorm):
        super().__init__()
        self.dim = dim
        self.reduction = nn.Linear(4 * dim, min(2 * dim, dim_max), bias=False)
        self.norm = norm_layer(4 * dim)
...
class BasicLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        for blk in self.blocks:  # SwinBlock3D
            x = blk(x, attn_mask)
        x = x.view(B, D, H, W, -1)
...
class SwinTransformer3D(nn.Module):
    ...
    def __init__(self,
        ...
        for i_layer in range(self.num_layers):
            layer = BasicLayer(
                ...
                dim=embed_dim if
                (downsample is None) else int(embed_dim * 2**i_layer),
    ...
    def forward(self, x):
        ...
        for layer in self.layers:  #BasicLayer:SwinBlock+Merging
...
class SwinTransformer3D_Encoder(nn.Module):
    ...
    def __init__(self,
        ...
        self.num_layers = len(depths)
        self.embed_dim = embed_dim
        self.embed_dim_max = embed_dim_max
        self.patch_norm = patch_norm
        self.frozen_stages = frozen_stages
        self.window_size = window_size
        self.patch_size = patch_size
        ...
        for i_layer in range(self.num_layers):
            layer = BasicLayer(
                # dim=int(embed_dim * 2**i_layer),
                dim=embed_dim if
                (downsample is None) else min(int(embed_dim *
                                                  2**i_layer), embed_dim_max),
                dim_max=embed_dim_max,
                depth=depths[i_layer],
                num_heads=num_heads[i_layer],
                window_size=window_size,
                mlp_ratio=mlp_ratio,
        ...
        self.num_features = min(int(embed_dim * 2**(self.num_layers - 1)),
                                embed_dim_max)
    ...
    def forward(self, x):
        ...
        x = self.pos_drop(x)
        # b, c, d, h, w = x.shape
        # out = [x.transpose(1, 2).reshape(-1, c, h, w)]

        for layer in self.layers:  #BasicLayer:SwinBlock+Merging
            x = layer(x.contiguous())
            # b, c, d, h, w = x.shape
            # out.append(x.transpose(1, 2).reshape(-1, c, h, w))
...
if __name__ == '__main__':
    # model = SwinTransformer3D(window_size=(3, 8, 8), patch_norm=True)
    model = SwinTransformer3D_Encoder(
        in_chans=6,
        embed_dim=64,
        embed_dim_max=256,
        depths=[2, 2, 2],
        num_heads=[4, 8, 16],
        window_size=(3, 8, 8),  #(nv,w,h)
        patch_norm=True,
        downsample=None)
    print(model)
    h, w = 512, 256

...
class _RenderWrapper(torch.nn.Module):
    ...
    def forward(self, rays, want_weights=False, visual_selection=False):
        if rays.shape[0] == 0:
            return (
...
class NeRFRenderer_vis(torch.nn.Module):
    """
    NeRF differentiable renderer
    ...
    def sample_coarse(self, rays):
        ...
        if not self.lindisp:  # Use linear sampling in depth space
            ...
            return 1 / (1 / (near + 1e-7) * (1 - z_steps) + 1 /
                        (far + 1e-7) * z_steps + 1e-7)  # (B, Kf)
    ...
    def sample_fine(self, rays, weights):
        ...
        weights = weights.detach() + 1e-7  # Prevent division by zero
        ...
        if not self.lindisp:  # Use linear sampling in depth space
            ...
            z_samp = 1 / (1 / (near + 1e-7) * (1 - z_steps) + 1 /
                          (far + 1e-7) * z_steps + 1e-7)  # (B, Kf)
        return z_samp
    ...
    def composite(self,
        ...
        with profiler.record_function("renderer_composite"):
                ...
                for pnts in split_points:
                    val_all.append(
                        model(pnts,
                              coarse=coarse,
                              visual_selection=visual_selection))
            for ii in range(len(val_all)):
                if torch.isnan(val_all[ii]).any():
                    print(20 * '-', 'debug', 20 * '-')
                    print('Problem variable:', 'val_all_', ii)
                    pdb.set_trace()
            # points = None
            viewdirs = None
            # (B*K, 4) OR (SB, B'*K, 4)
            ...
            alphas_shifted = torch.cat(
                ...
                -1)  # (B, K+1) = [1, a1, a2, ...]
            if torch.isnan(alphas_shifted).any():
                print(20 * '-', 'debug', 20 * '-')
                print('Problem variable:', 'alphas_shifted')
                pdb.set_trace()
            T = torch.cumprod(alphas_shifted, -1)  # (B)
            if torch.isnan(T).any():
                print(20 * '-', 'debug', 20 * '-')
                print('Problem variable:', 'T')
                pdb.set_trace()
            weights = alphas * T[:, :-1]  # (B, K)
            if torch.isnan(weights).any():
                print(20 * '-', 'debug', 20 * '-')
                print('Problem variable:', 'weights')
                pdb.set_trace()
            alphas = None
            alphas_shifted = None
            ...
            weights_msk = weights.clone()
            weights_msk[weights > 0.2] = 1
            weights_msk[weights <= 0.2] = 0
            points_w = weights_msk.unsqueeze(-1) * points
            if self.white_bkgd:
                # White background
    ...
    def _format_outputs(
        ...
        weights, rgb, depth, points = rendered_outputs
        if superbatch_size > 0:
            ...
            points = points.reshape(superbatch_size, -1, 3)
        ret_dict = DotMap(rgb=rgb, depth=depth, points=points)
        if want_weights:

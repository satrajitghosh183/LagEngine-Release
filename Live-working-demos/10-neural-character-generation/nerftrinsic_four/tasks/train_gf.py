...
def parse_args():
    ...
    parser.add_argument('--base_dir', type=str, default='./data/iFF')
    parser.add_argument('--scene_name', type=str, default='T1')
    ...
    parser.add_argument('--embedding_scale', type=int, default=10, help="fourier feature embedding_scale")
...
def eval_one_epoch(eval_c2ws, scene_train, model, focal_net, pose_param_net,
    ...
    for i in range(N_img):
        ...
        for rays_dir_rows in rays_dir_cam_split_rows:
            render_result = model_render_image(c2w=c2w, rays_cam=rays_dir_rows, t_vals=t_vals, near=scene_train.near, far=scene_train.far,
                                               H=H, W=W, fxfy=fxfy, model=model, perturb_t=False, sigma_noise_std=0.0, args=args, rgb_act_fn=rgb_act_fn)
...
def train_one_epoch(scene_train, optimizer_nerf, optimizer_focal, optimizer_pose, model, focal_net, pose_param_net,
    ...
    for i in ids:
        ...
        render_result = model_render_image(c2w=c2w, rays_cam=ray_selected_cam, t_vals=t_vals, near=scene_train.near, far=scene_train.far,
                                           H=H, W=W, fxfy= fxfy, model=model, perturb_t=True, sigma_noise_std=0.0, args=args, rgb_act_fn=rgb_act_fn)  # (N_select_rows, N_select_cols, 3)
...
def main(args):
    ...
    '''Create Folders'''
    exp_root_dir = Path(os.path.join('./logs/examples', args.scene_name))
    ...
    shutil.copy('./tasks/nertrinsic_four_train/train_gf.py', experiment_dir)
    ...
    '''Data Loading'''
    scene_train = DataloaderParameterLearning(base_dir=args.base_dir,
    ...
    print('Train with {0:6d} images.'.format(scene_train.N_imgs))
    ...
    pose_param_net = LearnPoseGF(num_cams=scene_train.N_imgs, init_c2w=None, embedding_scale=args.embedding_scale)

...
int main() {
    auto engine = std::make_shared<Engine>(1300, 800, "HPBD Cloth Simulation");

    Scene scene(engine);
    auto scenePtr = std::shared_ptr<Scene>(&scene, [](Scene*){}); // No-op deleter since scene is stack-allocated

    srand(time(0));

    auto camera = std::make_shared<OrbitCamera>(engine.get());
    ...
    auto light = std::make_shared<DirectionalLight>("sun", -1.0, 1.0, 0.0);
    scene.addDirectionalLight(light);

    auto shadowRenderer = std::make_shared<ShadowRenderer>(light, 4096, 4096);
    scene.addShadowRenderer(shadowRenderer);

    auto colorCorrection = std::make_shared<PostProcessing>("./assets/shaders/colorCorrection", engine.get());
    colorCorrection->onBeforeRenderObservable.add([colorCorrection]() {
    ...
    auto clothMaterial = std::make_shared<PbrMaterial>(scenePtr);
    ...
    bodyHelpers.insert({cloth, new PhysicsBodyHelper(cloth, scenePtr)});
    ...
    bodyHelpers.insert({cloth2, new PhysicsBodyHelper(cloth2, scenePtr)});
    ...
    auto cubeMaterial = std::make_shared<PbrMaterial>(scenePtr);
    ...
    bodyHelpers.insert({cubeBody, new PhysicsBodyHelper(cubeBody, scenePtr)});
    ...
    auto sphereMaterial = std::make_shared<PbrMaterial>(scenePtr);
    ...
    bodyHelpers.insert({sphereBody, new PhysicsBodyHelper(sphereBody, scenePtr)});

    auto bunnyMaterial = std::make_shared<PbrMaterial>(scenePtr);
    ...
    bodyHelpers.insert({armadilloBody, new PhysicsBodyHelper(armadilloBody, scenePtr)});
    ...
    auto groundMaterial = std::make_shared<PbrMaterial>(scenePtr);
